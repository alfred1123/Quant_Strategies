...
- **Put logic where it belongs.** Domain logic (e.g. promotion) goes in its own module/repo, not dumped into an orchestrator like the worker. Analyse ownership before placing code. Prefer that existing owner over a new class. `.cursor/rules/oop.mdc` says the same thing: the small fix the docs already name, not a migration onto `docs/architecture/oop-framework.md`. A control that is already on the page stays that control: a `disabled` prop or a "Coming feature" tooltip is the feature turned off, so re-enable it there and restore its old handler from git. Do not add a component for the same job.
...
**Editing a procedure body does nothing on its own.** Applied releases are archived, so no `runOnChange` changeset is standing by to re-apply a body you edit — a one-line change to a procedure reaches prod only through a **new changeset you write** that points at the file. Without one, the edit lands in git, the deploy goes green, and prod keeps the old definition. This reversed a scheme where twelve applied releases stayed included just to keep every procedure auto-redeployable; automatic re-apply hides the decision to deploy, and that is the shape that killed the 2026-08-16 migration. When you change a procedure, say so and write the changeset.
