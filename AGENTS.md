...
- **Put logic where it belongs.** Domain logic (e.g. promotion) goes in its own module/repo, not dumped into an orchestrator like the worker. Analyse ownership before placing code. Prefer that existing owner over a new class. `.cursor/rules/oop.mdc` says the same thing: the small fix the docs already name, not a migration onto `docs/architecture/oop-framework.md`.
