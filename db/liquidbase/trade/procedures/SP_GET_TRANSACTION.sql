-- Read TRADE.TRANSACTION rows (broker-confirmed fills) for one user.
--
...
--
-- Joins the current open deployment row for account / product context.
-- Exposes CREATED_AT as the fill timestamp (no separate TRANSACT_AT on fills).
-- Filter / ownership validation lives in Python.
CREATE OR REPLACE PROCEDURE TRADE.SP_GET_TRANSACTION(
...
BEGIN
    ...
    OUT_SQLMSG := '20';
    OUT_RESULT := 'sp_get_transaction_cursor';
        ...
        OPEN OUT_RESULT FOR
            SELECT t.TRANSACTION_ID,
             ...
             WHERE d.APP_USER_ID   = IN_APP_USER_ID
               AND t.DEPLOYMENT_ID = IN_DEPLOYMENT_ID
    ...
    CALL CORE_ADMIN.CORE_INS_LOG_PROC(
        'TRADE', 'SP_GET_TRANSACTION', V_LOG_START, NULL, V_OTHER_TEXT,
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        BEGIN
            ...
            RAISE WARNING '[SP_GET_TRANSACTION] % (SQLSTATE: %). Detail: %. Context: %. Params: %',
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'quant_app') THEN
    GRANT EXECUTE ON PROCEDURE TRADE.SP_GET_TRANSACTION(
        UUID, UUID, INTEGER,
