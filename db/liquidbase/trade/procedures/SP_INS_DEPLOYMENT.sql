...
CREATE OR REPLACE PROCEDURE TRADE.SP_INS_DEPLOYMENT(
    ...
    IN  IN_USER_ID                    TEXT,
    IN  IN_INITIAL_SCHEDULED_TS       TIMESTAMPTZ,
...
BEGIN
    ...
    IF IN_SCHEDULE_TM_INTERVAL_ID IS NOT NULL THEN
        IF V_VID = 1 THEN
            ...
            V_SCHEDULED_TS    := COALESCE(IN_INITIAL_SCHEDULED_TS, V_START_TS);
        ELSE
            SELECT ss.STATUS,
                   ss.SCHEDULED_TS
              INTO V_PRIOR_STATUS,
            ...
            IF IN_INITIAL_SCHEDULED_TS IS NOT NULL THEN
                V_SCHEDULED_TS := IN_INITIAL_SCHEDULED_TS;
            ELSE
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'quant_app') THEN
    GRANT EXECUTE ON PROCEDURE TRADE.SP_INS_DEPLOYMENT(
        UUID, UUID, UUID, INTEGER, INTEGER, INTEGER, TEXT, NUMERIC,
        CHAR(1), CHAR(1), TEXT, INTEGER, TEXT, TIMESTAMPTZ,
        OUT TEXT, OUT TEXT, OUT TEXT
    ) TO quant_app;
  END IF;
END $$;
