...
| `STRATEGY_VID` | `INTEGER` | Exact strategy version submitted. Join on `(STRATEGY_ID, STRATEGY_VID)`. |
...
### 6.3 `BT.STRATEGY` (updated — temporal versioning + `IS_BEST_IND`)

Queue rows store `(STRATEGY_ID, STRATEGY_VID)` at submission time. The worker joins on this exact pair so queue rows remain valid even if the user updates the strategy mid-queue. `STRAT_CURRENT_IND` is derived from `TRANSACT_TO_TS` (`'9999-12-31'` = current) in `FN_GET_QUEUE_FOR_TERMINAL` for UI display only. `IS_BEST_IND` marks the best-performing VID — see [Best-VID Promotion](best-vid-promotion.md).
