...
GET    /api/v1/trade/schedule-options          → Fitted cadence for a strategy   ✅ live
```
...
`PATCH` accepts `enabled`, `deployment_status`, `schedule_tm_interval_id`, and `qty`. Omitted fields keep their current value, with one exception: `enabled: true` on a `PAUSED` deployment also sets `ACTIVE` unless the request names a status, because an auto-pause writes `PAUSED` + disabled and the scheduler skips `PAUSED` rows even when enabled; sending `schedule_tm_interval_id: null` explicitly clears the schedule back to manual-only. `qty` cannot be cleared — `null` is **400**, and `qty ≤ 0` is **422**. A `qty` under the venue's own lot size is **400** on create and update alike, checked against the cached exchange rules ([api.md](../architecture/api.md#a-qty-the-venue-would-refuse-is-refused-while-it-can-still-be-fixed)); `DeploymentRow.min_qty` returns that lot size so the editor can say so first. A cadence other than the one the strategy was fitted on (read from its `CONFIG_JSON.tm_interval_id`) is rejected with **400**, but only when the request actually sets it — a stored value is never re-validated, so the kill switch always gets through. `GET /api/v1/trade/schedule-options?strategy_id=…&strategy_vid=…` returns that one acceptable id for the UI.
