...
2. **`TradeApplyPage`** — `ScheduleCell` in a Schedule column: the same dropdown, editable in place via `PATCH /trade/deployments/{id}`, with `next_due_at` beneath it when scheduled. Qty is the same shape: `QtyCell` edits `qty` in place on that PATCH (live rows confirm first).
