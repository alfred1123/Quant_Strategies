...
|-------|---------|----------|
| `/login` | `GuestOnly` | Shows `LoginPage`. If already authenticated, redirects to `/` (or the page the user came from via `location.state.from`). |
| `/` | `RequireAuth` | Shows `BacktestPage`. If not authenticated, redirects to `/login` (preserving the original URL in `state.from`). |
| `/*` | — | Catch-all redirects to `/`. |

Both wrappers call `useMe()` (`GET /api/v1/auth/me` via TanStack Query) and show a full-page spinner while the check is in-flight.
...
- On submit → `POST /api/v1/auth/login` → on 200, set `auth/me` query data, `navigate()` to `/` (or the page that triggered the auth redirect).
...
- Shows username.
- "Sign out" → `POST /api/v1/auth/logout` → clear local query cache → `navigate('/login', { replace: true })`.

(No "Change password…" item — admin handles it.)

### 9.4 Axios interceptor

Global response interceptor: on `401`, evict `auth/me` query data and call `window.location.replace('/login')`. Because the interceptor runs outside React (in the Axios layer), it uses the browser's History API directly rather than React Router's `navigate()`. This handles tokens that expire mid-session — the user is immediately bounced to the login page.
