...
The gaps that matter are operational, not stylistic. A worker process that dies after the loop has marked the job `RUNNING` is forgotten: the reaper logs the exit code and does not write `FAILED`, so the row stays `RUNNING` until the loop process itself restarts, and that restart fails every in-flight job, including the sibling. A schedule row that fails to advance after a successful apply is re-applied on later ticks, but a second order is not automatic: each attempt re-reads the book and holds when the position already matches the signal. The leftover risk is a position read that lags the fill. Python CI runs pytest and an offline Liquibase check. It does not run ruff or mypy. Both fail today. mypy's twenty errors include a latent `None` dereference on the apply report that no current caller hits. The production compose file sets `COOKIE_SECURE=0`, so the session cookie that authorizes trading is not marked Secure.
...
### 1. A stuck schedule re-applies every tick; a second fill needs a lagging position read

| | |
|---|---|
| Where | `quant/trade/scheduler/tick.py` lines 287–313. The guard that usually prevents a second order is `quant/trade/order_policy.py` lines 145–151 and `quant/trade/adapters/base.py` lines 117–155. |
| What's wrong | `_advance` writes the next `PENDING` slot only after the broker call has returned success. If that write throws, the deployment stays due (`TickOutcome.STUCK`) and the next tick calls apply again. The comment on lines 298–300 calls that a re-trade and says neither an in-flight lease nor an idempotent client order id exists. That comment overstates what the next tick does. `OrderRetryExecutor.execute` re-reads `adapter.get_position_qty` on every attempt and asks `TradeAdapter.intended_side` what to do. `intended_side` returns `HOLD` when the book already matches the signal: +1 while long, 0 while flat, −1 while short (lines 124–132 and 143, 149, 155). A re-apply with an unchanged signal and a current position places no order. |
| Why it matters | The practical effect of `STUCK` is that the deployment is applied again on every tick at the current signal, and each failed advance logs at `critical` (lines 301–305). A second fill still happens when the position read lags the fill. Two cases: an apply raises after the venue has accepted the order, and the in-pass retry (`_run_one`, lines 193–216) waits `DEFAULT_RETRY_BACKOFF_S` (5 seconds, line 45) then reads the old position; or a later tick whose venue position endpoint has not caught up. A client order id derived from `(deployment_id, scheduled_ts)` still closes that window. It is not required to stop the common case, which already holds. |
| Direction | Keep the client-order-id direction. Send that id on every attempt, including the replay, and treat "already filled" as success. Advance the schedule only after the venue has accepted the id. Do not describe today's `STUCK` path as a guaranteed double order. |
| Effort | Medium |
| How found | Read. Re-checked `execute` and `intended_side` against the `STUCK` path. |
...
| What's wrong | Frontend CI lints, type-checks, tests, and audits (`tests.yml` lines 77–88). The Python job does not. Default ruff reports 327 findings; about a third are the FastAPI `Depends(...)` pattern and should be ignored in a config rather than "fixed". mypy reports 20 errors. One is a latent type error, not a live crash: `quant/trade/live_apply.py` line 205 reads `result.message` when `result` is optional. The only caller that reaches `_report` with `result is None` is lines 110–113, and it passes `message=outcome.no_order_message` (a string; `quant/trade/order_policy.py` lines 61–64). The other caller (lines 125–127) runs only after that return, so `result` is set. The `else` branch on line 205 is unreachable today. The same mypy run flags untyped Redis replies as `Awaitable \| Any` in `quant/refdata/reader.py` lines 67 and 78 and `quant/trade/venue_limits.py` lines 167 and 183, and a swapped dict shape in `quant/trade/venue_limits.py` lines 71 and 84. |
| Why it matters | A future caller of `_report` that omits `message` would raise `AttributeError` while building the report. Nothing on the current apply path does that. The Redis errors mean the client is typed as sync-or-async, so a later `await` mistake type-checks. Without a CI gate, the next change can add either. |
| Direction | Add a ruff config that ignores `B008` on FastAPI routes, and a mypy config with `ignore_missing_imports`. Run both in the pytest job. Make line 205 `result.message if result else None` so the type matches the callers. Type the Redis client as the synchronous client. |
| Effort | Medium for the gate and the Redis typing. Small for the latent dereference. |
| How found | Ran ruff and mypy. The line 205 error is mypy `union-attr` on this commit. The call sites were re-read: the `None` path always passes `message`. |
...
Steps 1 through 4 are what the research team needs, in that order. Steps 5 and 6 are the boundary cleanup. Finding 2 (a non-zero worker exit left `RUNNING`) is a small fix and does not wait on this list. Finding 1 is a client-order-id follow-up for a lagging position read, not a guaranteed second order, so it does not jump this queue.

## Top 5 to do first

1. **Put indicators behind one registry and pass the full price frame through**, so a Donchian channel is a new class plus a REFDATA row. Today it is a new method on `TechnicalAnalysis`, a second edit in `Objective`, and a multi-factor path that drops High and Low (`quant/strategy/performance.py:183`).
2. **Reap a non-zero worker exit into `FAILED`** while the row is still `RUNNING` (`quant/queue/worker_loop.py` lines 192–199), and test exit code 1. Small, and independent of the registry work.
3. **Widen the signal call so a stateful rule can see the price path**, after the registry in step 1 exists. A trailing stop or hysteresis cannot be a `SignalDirection` static method of `(indicator, threshold)`.
4. **Add an identity sizer in front of PnL and keep live quantity on the deployment** until a sizer is explicitly chosen. Sizing has no object today. It would otherwise be edited into `Performance` and into `intended_side` separately.
5. **Give live orders a client id** derived from `(deployment_id, scheduled_ts)`, so a position read that lags a fill cannot send the order twice (`quant/trade/scheduler/tick.py` lines 297–313). A re-apply whose book already matches the signal already holds. This closes the remaining window. Medium, and not the first thing to do.
