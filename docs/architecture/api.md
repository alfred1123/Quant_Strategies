...
The table is the rule the code implements. A proposal to take cadence out of the key, keep date range on the version, and scope Best to one interval is [Backtest data hygiene](../design/2026-09-25-backtest-data-hygiene-proposal.md#strategy-identity). Adopting it would be a new decision.

### The range comes from what is captured

An exchange backtest reads `MARKET_DATA.PRICE_BAR`, so the stored bars bound it. `_fetch_exchange_df` refuses a range the store cannot cover rather than returning a shorter series under the requested label, and the config drawer reads `GET /price-bars/coverage` for every exchange series in the run (traded leg and cross-product factors) so the refusal is avoidable rather than a surprise: choosing an exchange series snaps Start and End to the captured overlap, a date typed outside it is flagged, and **Run Optimization applies that overlap rather than enqueueing a job the worker will refuse**. The overlap is the intersection: a BTC hourly snap of `2020-03-25 10:00` on an ETH factor whose first bar is `2021-03-15` is the window the worker refuses, so both series have to be in the set. A factor whose Data Source is still a provider is not in that set — `DEFAULT_CONFIG` used to pin `data_source: yahoo`, which is how the drawer kept naming only `btcusdt.crypto` after ETH was picked. Picking the factor product now moves a leftover provider onto the traded venue; a deliberate Yahoo / `^VIX` factor is set afterwards by changing Data Source. Provider sources are otherwise left alone — `Refresh dataset` refetches any window on demand, so what happens to be cached is not a floor.
