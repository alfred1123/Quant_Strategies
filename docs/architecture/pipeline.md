...
| `quant/market_data/fetcher.py` | `CcxtBarFetcher` | Paginated public-data fetch (`fetch_bars`) over ccxt's `fetch_ohlcv`. No API credentials — bars are public data. `defaultType` is the instrument's `ISSUE_TYPE` (Bybit: `spot` → `spot`, `future` → `linear`). |
