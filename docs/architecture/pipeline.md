...
| `quant/refdata/reader.py` | `RedisRefData` | Read-only REFDATA accessor backed by Redis. Checks `refdata:version` on every `get()` and rebuilds its local snapshot lazily on bump. Typed resolvers (`resolve_app_id`, `resolve_queue_status_id`, `get_interval_period`, `get_market_calendar`, `get_execute_offset`, …) live here so callers never parse raw rows. |
