...
|--------|-----------------|------|
| `quant/shared/db.py` | `DbGateway`, `open_pool` / `close_pools` | Sole owner of `psycopg` / `psycopg_pool` in `quant/`. Every call borrows from a process-wide `ConnectionPool` (decision #69). Entry points open the pool at boot and close it on shutdown. |
| `quant/refdata/reader.py` | `RedisRefData` | Read-only accessor for both snapshots. Checks `refdata:version` or `config:version` on `get()` and drops only that schema's rows. Typed resolvers (`resolve_app_id`, `resolve_queue_status_id`, `get_interval_period`, `get_market_calendar`, `get_execute_offset`, …) live here so callers never parse raw rows. |
| `quant/refdata/publisher.py` | `RefDataPublisher(DbGateway)` | The only Postgres → Redis writer for REFDATA and CONFIG. `publish("refdata")` and `publish("config")` each write that prefix and bump that version. `publish_all()` runs both, from FastAPI lifespan and `python -m quant.refdata.publisher`. |
...
| `quant/data/backtest_cache.py` | `BacktestCache(DbGateway)` | BT schema read/write via the process pool — split API: `read_payload()` (read-only, raises `CacheMissError` on miss) and `refresh_payload(fetcher=...)` (fetches the full range and inserts a new `API_REQUEST` version; SP write failures propagate). |
| `quant/data/instruments.py` | `InstrumentCache(DbGateway)` | INST schema cache via the process pool — products + vendor-symbol xrefs, exposed via `/api/v1/inst/products`. |
...
| `quant/market_data/repo.py` | `PriceBarRepo(DbGateway)` | MARKET_DATA SP wrappers via the process pool — coverage probe, range read, one-bar insert. |
| `quant/market_data/fetcher.py` | `CcxtBarFetcher` | Paginated public-data fetch (`fetch_bars`) over ccxt's `fetch_ohlcv`. No API credentials — bars are public data. `defaultType` comes from the order preset when that preset sets one (Bybit: `linear`). |
