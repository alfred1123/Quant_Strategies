...
|-----------|-------------|
| **Backtest Pipeline** | `quant/data/sources.py` → `quant/strategy/{indicators,signals}.py` → `performance.py` → `optimizer.py` → `walk_forward.py`, orchestrated by `quant.cli` (CLI) and `api/services/backtest.py` (web) |
