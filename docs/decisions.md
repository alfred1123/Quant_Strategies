...
| 26 | **Backtest queue infrastructure** | Postgres-backed FIFO queue, single process worker, `LISTEN/NOTIFY` for live SSE updates. **Not Kafka** (wrong primitive — high-throughput log fan-out, no existing dependency). **Not Redis/RQ** (new stateful service, breaks transactional coupling between job state and `BT.RESULT` writes). Re-evaluate Redis if submission rate or fan-out grows; re-evaluate Kafka only if a live market-data ingestion pipeline is added. | backtest-queue.md §20 |
